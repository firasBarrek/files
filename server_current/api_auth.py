...
class OAuthAuthorizeController(object):
    ...
    def _authorize(self):
	print('here _authorize ',self.request.authenticated_userid)
        scopes, credentials = self.oauth.validate_authorization_request(self.request.url)
        print('self.request.url',self.request.url)
        ...
        print('here client_id ',client_id)
        ...
        # logged-in user.
        if client.trusted:
	    print('ici trusted',client.redirect_uri)
            return self._authorized_response()
    ...
    def _render_web_message_response(self, redirect_uri):
        ...
        try:
            ...
            foundnetloc = re.search('%3A%2F%2F(.+?)&response_mode', request_filtred).group(1)
